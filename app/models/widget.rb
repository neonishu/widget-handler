...
class Widget
  ...
  def self.all(type, term = '', current_user)
    if type == 'visible'
      response = execute(:get, "/api/v1/widgets/visible?term=#{term}&client_id=#{CLIENT_ID}&client_secret=#{CLIENT_SECRET}", {})
    else
  ...
  def self.all_by_user(id, term = '', current_user)
    response = execute(:get, "/api/v1/users/#{id}/widgets?term=#{term}&client_id=#{CLIENT_ID}&client_secret=#{CLIENT_SECRET}", {}, { 'Authorization': "Bearer #{current_user.access_token}" })
    widgets = response[:data][:widgets].map { |widget| Widget.new(widget) }
    widgets
  rescue RestClient::UnprocessableEntity => e
    []
